...
async function initDevToolsExtension(): Promise<void> {
  try {
    ...
    editorDependencies.activateExtensionDependencies(
      mainConfig.extensionsDependencies
    );
...
async function handleDevToolsRequirements(/*isDevToolsProject: boolean*/): Promise<void> {
  ...
  const prerequisites: PrerequisitesInstalledReturn =
    await devtoolsPrerequisites.arePrerequisitesInstalled();
  log(
    "info",
    `SFMC Pre-Requisites ${
  ...
  if (prerequisites.prerequisitesInstalled) {
    const isDevToolsInstalled: boolean =
      await devtoolsInstaller.isDevToolsInstalled();
  ...
  log(
    "debug",
    `Missing Pre-requisites: [${prerequisites.missingPrerequisites}]`
  );
...
function handleStatusBarActions(action: string): void {
  ...
  switch (action.toLowerCase()) {
    ...
    default:
      log(
        "error",
        `main_handleStatusBarActions: Invalid Status Bar Action '${action}'`
      );
  }
}
...
function handleContextMenuActions(
  ...
  log(
    "debug",
    `Action: ${action} Number of Selected Files: ${selectedFiles.length}`
  );
  switch (action.toLowerCase()) {
    case "cmretrieve":
    ...
    default:
      log(
        "error",
        `main_handleContextMenuActions: Invalid Context Menu Action '${action}'`
      );
  }
}

async function getCredentialsBU(): Promise<
...
> {
  try {
    ...
    const credBUContent: string = await editorWorkspace.readFile(
      `${folderPath}/${mainConfig.credentialsFilename}`
    );
    ...
    if (parsedCredBUContent && "credentials" in parsedCredBUContent) {
      return Object.keys(parsedCredBUContent.credentials).reduce(
        (prev: {}, credential: string) => {
          ...
          if (businessUnits && Object.keys(businessUnits).length) {
            ...
            log(
              "error",
              `Could not find any business units for the credential '${credential}'`
            );
            return { ...prev };
          }
        },
...
async function changeCredentialsBU(): Promise<void> {
  ...
  if (credentialsBUList) {
    ...
    const credentialsOptions: InputOptionsSettings[] = Object.keys(
      credentialsBUList
    ...
    const selectedCredential:
      | InputOptionsSettings
      | InputOptionsSettings[]
      | undefined = await editorInput.handleQuickPickSelection(
    ...
    if (selectedCredential && !Array.isArray(selectedCredential)) {
      ...
      if (selectedCredential.id === mainConfig.allPlaceholder.toLowerCase()) {
        ...
        const businessUnitsList: string[] =
          credentialsBUList[selectedCredential.label];

        // Configures all business units names as selectable options
        const businessUnitOptions: InputOptionsSettings[] =
          businessUnitsList.map((businessUnit: string) => ({
    ...
    log(
      "error",
      "[main_changeCredentialsBU] Error: CredentialBU List is undefined."
    );
  }
}
...
async function handleDevToolsSBCommand(): Promise<void> {
  ...
  const devToolsCommandTypes: { id: string; title: string }[] =
    DevToolsCommands.getAllCommandTypes();

  if (devToolsCommandTypes) {
    // Configures all commandTypes names as selectable options
    const commandTypesOptions: InputOptionsSettings[] =
      devToolsCommandTypes.map(
    ...
    if (selectedCommandType && !Array.isArray(selectedCommandType)) {
      log(
        "debug",
        `User selected in ${selectedCommandType.label} DevTools Command type.`
      );
      ...
      const commandsOptions: InputOptionsSettings[] = commands.map(
        (command: DevToolsCommandSetting) => ({
      ...
      if (selectedCommandOption && !Array.isArray(selectedCommandOption)) {
        log(
          "debug",
          `User selected in ${selectedCommandOption.label} DevTools Command.`
        );
        if (devtoolsContainers.isCredentialBUSelected()) {
          ...
          if (selectedCredentialBU) {
            ...
            DevToolsCommands.runCommand(
              ...
              {
                bu: selectedCredentialBU.replace(
                  mainConfig.allPlaceholder,
                  "'*'"
                ),
              },
          ...
          if (DevToolsCommands.requiresCredentials(selectedCommandType.id)) {
            ...
            editorInput.handleShowNotificationMessage(
              "warning",
              `${mainConfig.messages.selectedCredentialsBU} '${lib.capitalizeFirstLetter(
                selectedCommandOption.id
              )}'...`,
              []
            );
...
async function initialize(): Promise<void> {
  ...
  const userResponse: string | undefined =
    await editorInput.handleShowOptionsMessage(
      mainConfig.messages.initDevTools,
      Object.keys(InstallDevToolsResponseOptions).filter((v) =>
        isNaN(Number(v))
      )
    );
  ...
  if (
    ...
    InstallDevToolsResponseOptions[
      userResponse as keyof typeof InstallDevToolsResponseOptions
    ]
  ) {
...
function getMCDevRelativePathComponents(
  relativePath: string
): DevToolsPathComponents {
  const [credentialName, businessUnit, metadataType, ...keys]: string[] =
    relativePath.split("/");
  return { credentialName, businessUnit, metadataType, keys };
}

function logUnsupportedMtdtTypeNotification(
  action: string,
  unsupportedMtdtTypes: string | string[]
) {
  ...
  editorInput.handleShowNotificationMessage(
    "error",
    mainConfig.messages.unsupportedMetadataType,
    []
  );
}

async function handleDevToolsCMCommand(
  action: string,
  ...
  try {
    ...
    const configureArgsProject = async (
      ...
      for (const filePath of selectedPaths) {
        ...
        log(
          "debug",
          `Project ${workspaceFolderPath === projectPath ? "is" : "is not"} the workspace folder.`
        );
        ...
        if (workspaceFolderPath !== projectPath) {
          ...
          const isSubFolderDevToolsProject: boolean = await isADevToolsProject(
            projectName + "/"
          );
          log(
            "debug",
        ...
        if (projectPath && !cmPath) {
          ...
          log(
            "debug",
            `Updated project path for '${action} "*"': ${projectPath}.`
          );
        }

        // When user clicks inside a retrieve or deploy folder
        if (cmPath) {
          let { credentialName, businessUnit, metadataType, keys } =
            getMCDevRelativePathComponents(cmPath.substring(1));
          let key: string = "";

          if (
            metadataType &&
            !DevToolsCommands.isSupportedMetadataType(action, metadataType)
          ) {
          ...
          if (filteredByBU.length) {
            ...
            args = [
              ...args.filter(
                ({ bu }: ArgsConfig) =>
                  bu !==
                  `${credentialName}/${businessUnit ? businessUnit : "*"}`
              ),
              newArgs,
            ];
          } else {
...
async function handleCopyToBuCMCommand(selectedPaths: string[]) {
  try {
    ...
    const actionOptionList: InputOptionsSettings[] = Object.values(
      CopyToBUInputOptions
    ).map((action: string) => ({ id: action, label: action, detail: "" }));

    const configuredSelectedPaths: DevToolsPathConfiguration[] =
      selectedPaths.map((path: string) => {
        const [_, relativeDevToolsPath]: string[] = path.split(
          /\/retrieve\/|\/deploy\//
        );
        return {
    ...
    }: SupportedMetadataTypeConfiguration = configuredSelectedPaths.reduce(
      (
        accObj: SupportedMetadataTypeConfiguration,
        configPath: DevToolsPathConfiguration
      ) => {
        if (
          DevToolsCommands.isSupportedMetadataType(
            "deploy",
    ...
    const selectedAction: InputOptionsSettings | undefined =
    ...
    if (selectedAction) {
      ...
      if (credentials) {
        ...
        if (!singleInstance) {
          const instanceOptions: InputOptionsSettings[] = instances.map(
            (instance: string) => ({
              id: instance,
              label: instance,
              detail: "",
        ...
        if (selectedInstance) {
          const buOptionsList: InputOptionsSettings[] = credentials[
            selectedInstance
          ...
          if (buOptions) {
            ...
            const buSelected: string[] = buOptions.map(
              (bu: InputOptionsSettings) => bu.id
            );
            ...
            const filePathsConfigured: FileCopyConfig[] = supportedMetadataTypes
                ...
                if (businessUnit) {
                  ...
                  if (file.isPathADirectory(absolutePath)) {
                    ...
                    const [currentFileExt]: string[] =
                      mainConfig.fileExtensions.filter((fileExt: string) =>
                          ...
                          mainConfig.fileExtensions
                            .filter(
                              (fileExtension: string) =>
                                !mainConfig.noCopyFileExtensions.includes(
                                  fileExtension
                                )
                            )
              ...
              .filter(
                (filePath: FileCopyConfig[] | undefined) =>
                  filePath !== undefined
              )
            ...
            const targetFilePaths: string[] = await file.copyFile(
              ...
              (error: any) => {
                if (error !== null) {
                  log(
                    "error",
                    `[main_handleCopyToBuCMCommand] Failed to copy file: ${error}`
                  );
                }
              }
            );
            ...
            if (unsupportedMetadataTypes.length) {
              logUnsupportedMtdtTypeNotification(
                "deploy",
                unsupportedMetadataTypes
              );
            }
        ...
        log(
          "error",
          `[main_handleCopyToBuCMCommand] Failed to retrieve DevTools credentials.`
        );
      }
    }
  } catch (error) {
