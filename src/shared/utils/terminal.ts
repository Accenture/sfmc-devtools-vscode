...
function executeTerminalCommand(commandRunner: TerminalCommandRunner): void {
	const childProcess: ChildProcess = spawn(commandRunner.command, commandRunner.args, {
		shell: true,
	...
	if (childProcess.stdout) {
		childProcess.stdout.on("data", (data: Buffer) =>
			commandRunner.handleResult(null, data.toString().trim(), null)
		);
	}

	if (childProcess.stderr) {
		childProcess.stderr.on("data", (data: Buffer) =>
			commandRunner.handleResult(data.toString().trim(), null, null)
		);
	}
