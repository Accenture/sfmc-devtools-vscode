...
async function copyFile(
	files: { sourceFilePath: string; targetFilePath: string }[],
	handleCopyFileError: (error: any) => void
): Promise<string[]> {
	try {
		const copiedFiles: Promise<string>[] = files.map(
			async ({ sourceFilePath, targetFilePath }: { sourceFilePath: string; targetFilePath: string }) => {
				...
				return new Promise<string>(resolve =>
					fs.cp(noDriveLetterSourceFilePath, noDriveLetterTargetFilePath, { recursive: true }, err =>
						err ? handleCopyFileError(err) : resolve(targetFilePath)
					)
				);
			}
		);
