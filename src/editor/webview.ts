...
async function create(config: WebviewConfig) {
	try {
		const panel: WebviewPanel = window.createWebviewPanel(config.id, config.title, ViewColumn.One, {
			enableScripts: true
		});

		let html: string = await editorWorkspace.readFile(
			lib.createFilePath([config.extensionPath, "src", "html", `${config.filename}.html`])
		);
		...
		const jsPath: string = lib.createFilePath([config.extensionPath, "src", "js", `${config.filename}.js`]);
