...
async function handleQuickPickSelection(
	...
	placeHolder: string,
	canPickMany: boolean
): Promise<InputOptionsSettings | InputOptionsSettings[] | undefined> {
...
async function handleInProgressMessage(
	local: string,
	callbackFn: (progress: Progress<{ message: string; increment?: number }>) => Promise<void>
) {
	await window.withProgress({ location: ProgressLocation[local as keyof typeof ProgressLocation] }, async progress =>
		callbackFn(progress)
	);
}
...
function handleShowNotificationMessage(level: keyof typeof NotificationMessage, message: string, actions: string[]) {
	...
	const notificationLevelFunctions: NotificationLevelFunctions = {
		info: (message: string, actions: string[]) =>
			window.showInformationMessage(message, ...actions, ...defaultActions),
		warning: (message: string, actions: string[]) =>
			window.showWarningMessage(message, ...actions, ...defaultActions),
	...
	callNotificationFunction(message, actions).then((response: string | undefined) => {
